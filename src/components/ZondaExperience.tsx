...
export default function ZondaExperience({ scrollYProgress }: ZondaExperienceProps) {
    ...
    return (
        <div className="absolute top-0 left-0 w-full h-full pointer-events-none z-10 flex flex-col justify-center">
            ...
            >
                <h1 className="text-7xl md:text-9xl font-black uppercase tracking-tighter text-white mb-4 italic transform -skew-x-12">
                    {CAR_DATA.hero.title}
                </h1>
                <p className="text-xl md:text-2xl text-lambo-yellow font-exo2 tracking-widest mb-8 border-l-4 border-lambo-yellow pl-4">
                    {CAR_DATA.hero.price}
                </p>
            </motion.div>
            ...
            >
                <div className="max-w-xl frosted-glass p-10 border-l-4 border-lambo-yellow">
                    <h2 className="text-4xl md:text-6xl font-bold uppercase mb-6 text-white italic">
                        {CAR_DATA.design.title}
                    </h2>
                    <p className="text-lg md:text-xl leading-relaxed text-gray-200 font-inter font-light">
                    ...
                    <div className="flex flex-col gap-8">
                        {CAR_DATA.engine.specs.map((spec, index) => (
                            <div key={index} className="flex flex-col items-end group">
                                <span className="text-xs uppercase tracking-[0.3em] text-gray-500 mb-2 group-hover:text-lambo-yellow transition-colors">
                                    {spec.label}
                                </span>
                                <span className="text-4xl md:text-6xl font-exo2 font-black text-white group-hover:text-lambo-yellow transition-colors">
                                    {spec.value}
                                </span>
                            </div>
                        ))}
